...
% Added for 500 mm 8-pole mini prototype analysis

machine.Npole = 8;                        % Number of Poles
machine.pole_angle = 360 / machine.Npole;   % One pole pitch angle in degrees

machine.Nrpm = 100;                          %Rated rotational speed of the generator (RPM)
machine.f_electrical = (machine.Nrpm /60)* (machine.Npole/2);   %Machine induced voltage frequency (Hz)

%Number of axial stacks
machine.Nstacks = 2; % Number of generator stator stacks in the axial direction
...
HTS.R_mean = 175/1000;     %meter, Mean radius of the HTS rotor
HTS.coil_length = 102/1000;  %meters, Straight section length of rotor HTS coil
...
stator.coil_length = 102/1000;  %meters, Straight section length of the stator coils, independent but correlated with HTS.coil_length, stator.R_inner, Stator.R_outer
...
stator.coil_width_to_coil_pitch_ratio = 0.164;  %Coil width to pole pitch ratio (at mean radius), <0.5 but considering inner radius 0.45 is a more realistic limit
...
end_winding_type = 1; %End winding type: 0= no end winding, 1 = only vertical, 3 = vertical with diagonal ones (three in total)

end_winding_gap = 0.01;   %Z distance from the last stage of coils (the name can be modified)
